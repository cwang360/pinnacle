  ...
  for(Hand hand : leap.getHands()) {
    if(hand.isLeft()){
      for(Finger finger : hand.getFingers()){
        ...
        deltaPos.set(finger.getType(), new Double(currPos.mag() - posFrames.get(finger.getType()).remove().mag()));
      ...
      for(Finger finger : hand.getFingers()){
        PVector currPos = finger.getPosition();
        deltaPos.set(finger.getType()+5, new Double(currPos.mag() - posFrames.get(finger.getType()+5).remove().mag()));
